...
class StreamsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create stream" do
    assert_difference('Stream.count') do
      post streams_url, params: { stream: { description: @stream.description, title: @stream.title } }
    end
  ...
  test "should update stream" do
    patch stream_url(@stream), params: { stream: { description: @stream.description, title: @stream.title } }
